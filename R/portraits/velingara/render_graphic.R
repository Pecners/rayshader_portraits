...
y <- r * sin(t) ^ 3
x <- ((50 * r/16) * cos(t) - (4 * r/16) * cos(2 * t) + (.25 * r/16) * cos(3 * t))

coords <- c(13.029352, -14.150246)

data <- tibble(x = x + coords[2], 
               y = y + coords[1]) %>%
  st_as_sf(., coords = c("x", "y"), crs = 4326) %>%
  st_transform(crs = 31028) |> 
...
render_camera(phi = 80, zoom = .55, theta = 0)
