...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("new_mexico", "_", " ")))
...
st_d <- st_transform(st_d, crs = 2257)
...
render_camera(phi = 60, zoom = .85, theta = 5)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(140, 140, 145, 145)),
