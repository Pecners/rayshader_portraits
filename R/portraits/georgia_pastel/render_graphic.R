...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("georgia", "_", " ")))
...
colors <- c("#FFE5B4", "#FDA26B", "#FC8861", "#FA6F72", "#EB6758") |> 
  lighten(.1)

colors <- c("#FBCEB1", "#FFBF99", "#FDA785", "#F89A78", "#FF9168")


swatchplot(colors)

texture <- grDevices::colorRampPalette(colors[c(3:1, 4:5)] |> rev(), bias = 3)(256)
texture <- grDevices::colorRampPalette(colors[2:5], bias = 2)(256)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(200, 200, 195, 195)),
    ...
    # ground_material = rayrender::diffuse(color = c1[5],
    #                                      noisecolor = c1[4]),
    ground_material = rayrender::microfacet(roughness = .55,
                                            eta = c(1, .1, .75), 
                                            kappa = c(.1, .75, 1))
  )
  end_time <- Sys.time()
