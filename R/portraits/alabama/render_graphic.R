...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("alabama", "_", " ")))
...
c1 <- scico(n = 10, palette = "vanimo", direction = -1)
swatchplot(c1)
colors <- c1[c(10:7)]
swatchplot(colors)

texture <- grDevices::colorRampPalette(colors, bias = .33)(256)
...
render_camera(phi = 45, zoom = .75, theta = -10)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(250, 250, 240, 240)),
    lightcolor = c(c1[2], "white", c1[10], "white"),
    ...
    ground_material = rayrender::diffuse(color = c2[4], noisecolor = c2[2])
  )
  end_time <- Sys.time()
