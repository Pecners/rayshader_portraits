...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("wyoming", "_", " ")))
...
st_d <- st_transform(st_d, crs = 2863)
...
c1 <- natparks.pals("Glacier", n = 10, direction = -1)
colors <- c1
swatchplot(colors)

texture <- grDevices::colorRampPalette(colors, bias = .25)(256)
...
render_camera(phi = 35, zoom = .85, theta = -10)
...

{
  ...
  render_highquality(
    ...
    lightcolor = c(colors[3], "white", colors[1], "white"),
