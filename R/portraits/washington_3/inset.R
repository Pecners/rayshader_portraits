...
loc_plot <- ggplot(data = done_world) +
  geom_sf(data = water, fill = header$colors[4], color = NA) +
  geom_sf(aes(fill = ifelse(name == "United States", colors[7], colors[8])), 
          size = .1, color = NA) +
  geom_sf_text(aes(label = ifelse(name == "United States", "USA", "")),
               size = 10, color = colors[5], family = "Cinzel",
               fontface = "bold",
               nudge_x = -1e5, nudge_y = 1e5) +
  geom_sf(data = wa, fill = text_color, color = NA) +
  geom_sf_label(data = wa, aes(label = NAME),
               nudge_x = 4e5, hjust = 0, family = "Cinzel",
               color = text_color, size = 9, fontface = "bold",
               label.size = unit(0, "cm"), fill = alpha("white", .75),
...
data <- st_read("data/nps_boundary/nps_boundary.shp") |>
  filter(PARKNAME %in% c("Olympic", "Mount Rainier", "North Cascades")) |> 
  st_transform(crs = 32148)

wa |> 
  ggplot() + 
  with_shadow(geom_sf(fill = colors[4], color = "grey50"), colour = "grey50",
              x_offset = 0, y_offset = 30, sigma = 50) + 
  geom_sf_text(aes(label = str_to_upper(NAME)), color = colors[1], 
               family = "Cinzel", fontface = "bold",
               nudge_y = 25000, nudge_x = 0, size = 14) +
  geom_sf(data = data, color = colors[7], fill = colors[7]) +
...
image_write(both, "images/washington_3/final_inset.png")
