...
gl_buff <- gl |> 
  st_buffer(dist = 1609.34 * 10) |> 
  st_as_sf() |> 
  select("geometry" = x)

wkt_st <- st_as_text(gl_buff[[1,"geometry"]])

ff <- c(
  "data/kontur/kontur_population_US_20220630.gpkg",
  "data/kontur/kontur_population_CA_20220630.gpkg"
)

data <- map_df(ff, function(f) {
  st_read(f, wkt_filter = wkt_st)
})
...
st_d <- st_join(data, gl_buff, left = FALSE)
st_d <- st_transform(st_d, crs = 3174)
...
render_camera(phi = 60, zoom = 1, theta = -10)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(285, 285, 290, 290)),
    # lightcolor = c(colors[6], "white", colors[9], "white"),
    # lightintensity = c(750, 50, 1000, 50),
    # lightaltitude = c(20, 80, 20, 80),
    # 
    # lightdirection = 290,
    lightcolor = c("white", colors[1], "white", colors[5]),
    lightintensity = c(100, 600, 100, 600),
    lightaltitude = c(80, 20, 80, 20),
