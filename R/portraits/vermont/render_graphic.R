...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("vermont", "_", " ")))
...
data |> 
  ggplot() +
  geom_sf() +
  geom_sf(data = st, color = "red", fill = NA)
...
texture <- grDevices::colorRampPalette(cf, bias = 2)(256)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(120, 120, 125, 125)),
    ...
    ground_material = rayrender::microfacet(roughness = .45, 
                                            eta = c(.25, .75, .25),
                                            kappa = c(0, 0, 1))
  )
  end_time <- Sys.time()
