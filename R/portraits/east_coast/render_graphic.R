...
cont <- s |> 
  filter(NAME %in% eastern_sb)

cont |> 
  ggplot() +
  geom_sf()

coast <- ne_download(type = "coastline", category = "physical",
                     scale = "large", returnclass = "sf")  %>%
  st_transform(., crs = d_crs)

east_coast <- st_intersection(coast, cont) |> 
  st_union() |> 
...
wkt_st <- st_as_text(east_coast[[1, "geometry"]])
...
st_d <- st_join(data, east_coast, left = FALSE)
st_d <- st_transform(st_d, crs = 5070)
