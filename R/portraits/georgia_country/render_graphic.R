...
st_dd <- st_read("data/kontur/kontur_population_GE_20220630.gpkg")

countries_sf <- map_df(c("GEO", "RUS", "AZE", "ARM", "TUR"), function(i) {
  d_ <- gadm(country = i, path = i, level = 0) |> 
    st_as_sf() |> 
    st_transform(crs = st_crs(st_dd))
  
  if(i != "GEO") {
    d_ <- st_cast(d_, "MULTILINESTRING")
  }
  
  return(d_)
})

buff <- st_bbox(countries_sf[1,]) |> 
  st_as_sfc() |> 
  st_buffer(200000)

clipped <- st_intersection(countries_sf, buff)
...
render_camera(phi = 45, zoom = .65, theta = 20)
