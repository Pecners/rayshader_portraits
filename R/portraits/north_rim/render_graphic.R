...
walk(seq(from = 0, to = 360, by = 2), function(i) {
  ...
  res <- mean(round(terra::res(zelev))) / 7
  ...
  # GitHub difficult. 
  outfile <- str_to_lower(glue("images/{map}/{map}_{pal}_z{z}_{i}.png"))
...
files <- list.files("images/north_rim")
f_order <- tibble(
  f = files,
...
png_files <- paste("images/north_rim", f_order, sep = "/")

av::av_encode_video(png_files, output = "images/north_rim/full_size_slow.mp4", framerate = 12)
