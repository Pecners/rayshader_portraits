...
s <- places(state = c("Wisconsin", "Missouri"))
...
mke_d <- st_join(data, mke, left = FALSE)
stl_d <- st_join(data, stl, left = FALSE)
...
mke_x <- st_coordinates(mke_c$geometry) |> 
  as_tibble() |> 
  pull(X)

stl_x <- st_coordinates(stl_c$geometry) |> 
  as_tibble() |> 
  pull(X)
...
mke_y <- st_coordinates(mke_c$geometry) |> 
  as_tibble() |> 
  pull(Y)

stl_y <- st_coordinates(stl_c$geometry) |> 
  as_tibble() |> 
  pull(Y)

diff_y <- mke_y - stl_y

# Shift the STL north to MKE 

stl_shifted <- map_df(1:nrow(stl_d), function(i) {
  tmp <- stl_d[i,]
...
st_d <- bind_cols(stl_shifted, population = stl_d$population) |> 
  bind_rows(mke_d |> 
...
colors <- met.brewer("Tam", n = 10, direction = -1)

swatchplot(colors)

texture <- grDevices::colorRampPalette(colors[2:10], bias = 2)(256)
...
render_camera(phi = 60, zoom = .85, theta = -10)
