...
data <- st_read("data/nps_boundary/nps_boundary.shp") |>
  filter(str_detect(PARKNAME, str_to_title(map))) 
...
t1 <- grDevices::colorRampPalette(colors[1:3])(256)
t2 <- grDevices::colorRampPalette(colors[4:8], bias = .75)(256)
swatchplot(t2)

custom <- height_shade2(mat, t1, t2, split = 3500, keep_user_par = TRUE)
