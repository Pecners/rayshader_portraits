...
y <- r * sin(t) ^ 3
x <- -1 * ((50 * r/16) * cos(t) - (4 * r/16) * cos(2 * t) + (.25 * r/16) * cos(3 * t))

coords <- c(-11.401515762405115, 21.127630356438505)

data <- tibble(x = x + coords[1], 
               y = y + coords[2]) %>%
  st_as_sf(., coords = c("x", "y"), crs = 4326) %>%
  st_transform(crs = 3345) |> 
...
mat %>%
  ...
  height_shade(texture = grDevices::colorRampPalette(colors, bias = 1.25)(256)) %>%
