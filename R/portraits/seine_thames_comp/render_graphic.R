...
map <- "seine_thames_comp"

# Kontur data source: https://data.humdata.org/organization/kontur

# This script reads in data for the Seine and the Thames, then shifts the
# Thames polygons so they are south of the Seine.

data_t <- st_read("data/kontur/kontur_population_GB_20220630.gpkg")
data_s <- st_read("data/kontur/kontur_population_FR_20220630.gpkg")

rivers <- st_read("data/ne_10m_rivers_lake_centerlines/ne_10m_rivers_lake_centerlines.shp")

# Thames buffer
thames <- rivers |> 
  filter(name == "Thames") |> 
  st_transform(crs = st_crs(data_t))
...
# Set up normal datasets masked to the buffers

st_dd_s <- st_intersects(s_buff, data_s)
st_d_s <- data_s[st_dd_s[[1]],]

st_dd_t <- st_intersects(t_buff, data_t)
st_d_t <- data_t[st_dd_t[[1]],]
...
# Determine how far to move along X

seine_x <- st_coordinates(sc$geometry) |> 
  as_tibble() |> 
  pull(X)

thames_x <- st_coordinates(tc$geometry) |> 
  as_tibble() |> 
  pull(X)

diff_x <- seine_x - thames_x

# Determine how far to move along Y

seine_y <- st_coordinates(sc$geometry) |> 
  as_tibble() |> 
  pull(Y)

thames_y <- st_coordinates(tc$geometry) |> 
  as_tibble() |> 
  pull(Y)

diff_y <- thames_y - seine_y

# Shift the Thames south of the Seine. This isn't 
# very efficient, probably a better way to do this.

thames_shifted <- map_df(1:nrow(st_d_t), function(i) {
  tmp <- st_d_t[i,]
  ...
  tmp |> 
    ...
    mutate(X = X  + diff_x * .8,
           Y = Y - diff_y * 1.4) |> 
    st_as_sf(coords = c("X", "Y"), crs = st_crs(data_t)) |> 
...
thames_shifted |> 
  ggplot() +
  geom_sf(color = "blue") +
  geom_sf(data = st_d_s, color = "red")

# Combine Seine and shifted Thames datasets
st_both <- bind_cols(thames_shifted, population = st_d_t$population) |> 
  bind_rows(st_d_s |> 
...
# Everything below is back to normal for these scripts
