...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("south_dakota", "_", " ")))
...
st_d <- st_transform(st_d, crs = 3455)
...
render_camera(phi = 35, zoom = .95, theta = -10)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(290, 290, 300, 300)),
