...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("nebraska", "_", " ")))
...
texture <- grDevices::colorRampPalette(colors[1:4], bias = 3)(256)
...
render_camera(phi = 45, zoom = .9, theta = -10)
...

{
  ...
  render_highquality(
    ...
    lightcolor = c(lighten(colors[1], .25), "white", lighten(colors[4], .25), "white"),
    lightintensity = c(500, 50, 750, 50),
    ...
    ground_material = rayrender::diffuse(color = "#fff7d9", noisecolor = "#ffcc98")
  )
  end_time <- Sys.time()
