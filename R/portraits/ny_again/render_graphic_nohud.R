...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("new_york", "_", " ")))

canals <- opq(st_transform(st, crs = 4326) |> st_bbox()) |> 
  add_osm_feature(key = "waterway", value = "canal") |> 
  osmdata_sf()

erie <- canals$osm_multilines |> 
  filter(name == "Erie Canal") |> 
...
less_erie <- st_difference(st, st_buffer(erie, 1609.34 * 5))

wkt_st <- st_as_text(less_erie[[1,"geometry"]])
...
st_d <- st_join(data, less_erie, left = FALSE)
...
# GitHub difficult. 
outfile <- str_to_lower(glue("images/{map}/{map}_{pal}_nohud.png"))
...
saveRDS(list(
  ...
  outfile = outfile,
  coords = coords
), glue("R/portraits/{map}/header_nohud.rds"))

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(185, 185, 178, 178)),
    lightcolor = c(colors[1], "white", colors[5], "white"),
    ...
    ground_material = rayrender::microfacet(roughness = c(.2, .7), 
                                            eta = c(1, .75, .1), 
                                            kappa = c(.1, .75, 1))
  )
  end_time <- Sys.time()
