...
colors <- c(
  lighten("#00296b", seq(from = 0, to = .8, by = .4)),
  darken("green4", .4),
  "green4",
  lighten("green4", seq(from = .2, to = 1, by = .2))
)

swatchplot(colors)


texture <- grDevices::colorRampPalette(colors, bias = .66)(256)
...
shadow_depth <- min(mat, na.rm = TRUE) - 250

# setting resolution to about 5x for height
res <- mean(round(terra::res(zelev))) / 10
