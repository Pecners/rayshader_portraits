...
st <- s |> 
  filter(NAME == str_to_title(str_replace_all("utah", "_", " ")))
...
st_d <- st_transform(st_d, crs = 2281)
...
colors <- c(c1[6:10], "#5ad6ff")
swatchplot(colors)

texture <- grDevices::colorRampPalette(colors[2:6], bias = 6)(256)
...

{
  ...
  render_highquality(
    ...
    lightcolor = c(colors[6], "white", colors[3], "white"),
    ...
    ground_material = rayrender::diffuse(color = colors[1], noisecolor = colors[4])
  )
  end_time <- Sys.time()
