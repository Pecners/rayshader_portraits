...
mex <- st_read("data/kontur_population_MX_20220630.gpkg")

data <- bind_rows(data, mex)
...
longs <- map_df(c(-106, -111, -116), function(i) {
  tibble(x = i,
         y = c(32, 41)) |> 
...
lats <- map_df(c(34.5, 38.5), function(i) {
  tibble(y = i,
         x = seq(from = -117, to = -105, by = 1)) |> 
    st_as_sf(coords = c("x", "y"), crs = 4326) |> 
...
render_camera(phi = 45, zoom = .8, theta = 0)
