...
st_counties |> 
  ggplot() +
  geom_sf() +
  geom_sf(data = these_counties, fill = "red", color = "white")


wkt_st <- st_as_text(these_counties[[1,"geometry"]])
...
st_dd <- st_join(data, these_counties, left = FALSE)
...

{
  ...
  render_highquality(
    ...
    preview = FALSE,
    light = TRUE,
    lightdirection = rev(c(210, 210, 215, 215)),
    ...
    ground_material = rayrender::diffuse(color = colors[3], noisecolor = "white")
  )
  end_time <- Sys.time()
