...
mix <- mixcolor(alpha = seq(from = 0, to = 1, by = .25), color1 =  hex2RGB(c1[7]), 
                color2 = hex2RGB(c1[6])) |> 
  hex() |> 
  swatchplot()

mix1 <- mixcolor(alpha = seq(from = .1, to = .9, by = .1), color1 =  hex2RGB(c1[6]), 
                     color2 = hex2RGB("#B0C4DE")) |> 
  hex() |> 
  swatchplot()

mix2 <- mixcolor(alpha = seq(from = .1, to = .9, by = .2), color1 =  hex2RGB("#B0C4DE"), 
...
mat %>%
  ...
  height_shade(texture = grDevices::colorRampPalette(colors, .75)(256)) %>%
