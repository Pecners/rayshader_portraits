...
texture <- grDevices::colorRampPalette(colors |> rev(), 1.5)(256)
...
render_camera(phi = 80, zoom = .9, theta = 0)
...

{
  ...
  render_highquality(
    ...
    # lightaltitude = c(20, 80, 20, 80),    # All it takes is accidentally interacting with a render that takes
    ...
    ground_material = rayrender::microfacet(roughness = .15, 
      eta = c(1, .75, .1), 
      kappa = c(.1, .75, 1))
  )
  end_time <- Sys.time()
