...
render_camera(phi = 80, zoom = .6, theta = 0)
...
if (!dir.exists(glue("images/{map}/gif"))) {
  dir.create(glue("images/{map}/gif"))
}
...
# GitHub difficult. 
outfile <- str_to_lower(glue("images/{map}/gif/{map}_{pal}_z{z}.png"))
...
walk(seq(from = 0, to = 360, by = 3), function(i) {
  ...
  start_time <- Sys.time()
  cat(glue("{i} Start Time: {start_time}"), "\n")
  render_highquality(
    # We test-wrote to this file above, so we know it's good
    filename = str_to_lower(glue("images/{map}/gif/{map}_{pal}_z{z}_{i}.png")),
...
files <- list.files("images/badlands/gif")
f_order <- tibble(
  f = files,
  order = as.numeric(str_extract(files, "\\d+(?=\\.png)"))
) |> 
  arrange(order) |> 
  pull(f)

png_files <- paste("images/badlands/gif", f_order, sep = "/")

av::av_encode_video(png_files, output = "images/badlands/full_size_slow.mp4", framerate = 5)
