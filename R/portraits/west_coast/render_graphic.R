...
western_sb <- c("Washington",
                "Oregon",
                "California")

cont <- s |> 
  filter(NAME %in% western_sb)

coast <- ne_download(type = "coastline", category = "physical",
                     scale = "large", returnclass = "sf")  %>%
  st_transform(., crs = d_crs)

west_coast <- st_intersection(coast, cont) |> 
  st_union() |> 
  st_buffer(1609.34 * 10) |> 
  st_as_sf() |> 
  select("geometry" = x)

wkt_st <- st_as_text(west_coast[[1,"geometry"]])
...
st_d <- st_join(data, west_coast, left = FALSE)
...
render_camera(phi = 60, zoom = .85, theta = -30)
